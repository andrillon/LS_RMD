...
for nF=1:length(files)
    ...
    if max(all_Waves(:,2))<5
        warning('subj has less than 5 blocks so skipping')
        continue;
    else
        fprintf('subj has %g blocks, taking the first 5\n',max(all_Waves(:,2)))
        all_Waves(all_Waves(:,2)>5,:)=[];
    end
...
offset=4;
clims=[5.6 7.9; 19 37; 250 450; 250 450];
figure;
subplot(2,2,1);
temp_topo=[];
for nCh=1:length(layout.label)-2
...
temp_topo=[];
for nCh=1:length(layout.label)-2
    [~,~,stats]=ranksum(all_nSW_perE(all_nSW_perE(:,4)==0 & all_nSW_perE(:,3)~=2,offset+match_str(newlabels,layout.label(nCh))),...
...
temp_topo=[];
for nCh=1:length(layout.label)-2
    [~,~,stats]=ranksum(all_P2P_perE(all_nSW_perE(:,4)==0 & all_nSW_perE(:,3)~=2,offset+match_str(newlabels,layout.label(nCh))),...
        all_P2P_perE(all_nSW_perE(:,4)==1 & all_nSW_perE(:,3)~=2,offset+match_str(newlabels,layout.label(nCh))));
    temp_topo(nCh)=stats.zval;
end
temp_topo(match_str(layout.label,{'TP7','TP8'}))=NaN;
simpleTopoPlot_ft(temp_topo', layout,'on',[],0,1);
...
temp_topo=[];
for nCh=1:length(layout.label)-2
    [~,~,stats]=ranksum(all_DnS_perE(all_nSW_perE(:,4)==0 & all_nSW_perE(:,3)~=2,offset+match_str(newlabels,layout.label(nCh))),...
        all_DnS_perE(all_nSW_perE(:,4)==1 & all_nSW_perE(:,3)~=2,offset+match_str(newlabels,layout.label(nCh))));
    temp_topo(nCh)=stats.zval;
end
temp_topo(match_str(layout.label,{'TP7','TP8'}))=NaN;
simpleTopoPlot_ft(temp_topo', layout,'on',[],0,1);
...
temp_topo=[];
for nCh=1:length(layout.label)-2
    [~,~,stats]=ranksum(all_UpS_perE(all_nSW_perE(:,4)==0 & all_nSW_perE(:,3)~=2,offset+match_str(newlabels,layout.label(nCh))),...
        all_UpS_perE(all_nSW_perE(:,4)==1 & all_nSW_perE(:,3)~=2,offset+match_str(newlabels,layout.label(nCh))));
    temp_topo(nCh)=stats.zval;
end
temp_topo(match_str(layout.label,{'TP7','TP8'}))=NaN;
simpleTopoPlot_ft(temp_topo', layout,'on',[],0,1);
...
figure;
subplot(1,2,1);
simpleTplot(1:5,temp_plot,0,'k',0,'-',0.5,1,0,1,2);
xlabel('Block Number')
ylabel('SW density')
format_fig;
...
subplot(1,2,2);
hp=[];
[~,hp(1)]=simpleTplot(1:5,temp_plot(agegroup==0,:),0,[0 0 1],0,'-',0.5,1,0,1,2);
[~,hp(2)]=simpleTplot(1:5,temp_plot(agegroup==1,:),0,[1 0 0],0,'-',0.5,1,0,1,2);
xlabel('Block Number')
ylabel('SW density')
legend(hp,{'Young','Old'})
format_fig;
