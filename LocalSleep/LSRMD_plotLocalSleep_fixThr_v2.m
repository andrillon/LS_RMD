...
%             thr_Wave(nFc)=paramSW.fixThr;
%         else
%             thr_Wave(nFc)=prctile(thisE_Waves(:,paramSW.AmpCriterionIdx),paramSW.prticle_Thr);
%         end
% %     end
% end
...
for nF=1:length(files)
    ...
    for nE=1:size(data.label,1)
        ...
        slow_Waves=[slow_Waves ; thisE_Waves(temp_p2p>paramSW.fixThr,:)];
    end
...
figure;
temp_topo=[];
mygroups=unique(group);
for ngroup=1:length(mygroups)
    subplot(1,length(mygroups),ngroup);
    temp_topo{ngroup}=[];
    for nCh=1:length(layout.label)-2
        temp_topo{ngroup}(nCh)=nanmedian(all_nSW_perE(ismember(all_nSW_perE(:,1),num_ch_blocks(group==mygroups(ngroup),1)) & all_nSW_perE(:,2)<10,2+match_str(newlabels,layout.label(nCh))),1);
    end
